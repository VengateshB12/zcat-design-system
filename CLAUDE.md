...
**Live Figma component definitions are the source of truth for implementation.** Composition decides the design; the live library decides how it is built. The order is:

```
Design intent → Composition → Design role → Canonical zcat component
              → Live component properties / variants / tokens → Figma implementation
```

Reference files describe the library; they are not the library. When a reference file and the live `componentPropertyDefinitions` disagree, **live wins** — and the disagreement is a bug to fix via `references/library-audit.md`, not a judgement call to route around. Never derive variant or property values from a component's *description* text: descriptions in the library are stale and demonstrably contradict the real definitions.

**Screen Polish & Verify is mandatory, and the design critique is a GATE.** After building each screen and before showing it to the user, run the comprehensive audit + improvement loop: structural integrity, component correctness, wireframe completeness (inch by inch), and design quality. Then critique the **rendered screenshot** independently — as a designer seeing it for the first time, with no knowledge of your plan. "The composition direction was good, therefore the UI is good" is FORBIDDEN reasoning: a correct plan is not evidence of a resolved screen. Either fix what the critique finds and re-screenshot (max 2 rounds), or record verbatim: *"No meaningful design improvement identified; keeping the current composition."* Never invent changes just to look more creative. See `rules-design-composition.md` "Screen Polish Patterns" and "Card Composition Recipes".
...
- Divider component IS importable — key `ae8ace032eb5e3ff8b86424a97be7a3728bde3bd` (component_set, State=Default/Active/Completed/Disabled). Use it for dividers. Only build a manual 1px frame when you need a plain rule the component's states don't fit, and bind its fill to `CARDS/Borders/Default`
...
- Manual elements MUST use variables — when building manual frames, dividers, section separators, or any non-component element: ALL fills/strokes must be bound to zcat color variables imported by key (`BODY/Text/Static/*`, `BODY/Background/Static/*`, `BODY/Border/Static/*`, `CARDS/Bg Default/*`, `CARDS/Borders/*` — see the key table in zcat.md), ALL spacing (padding, gap) must use values from the spacing scale (0-128px, even numbers), ALL text must use one of the verified zcat text styles imported by key. No raw hex colors, no arbitrary spacing, no custom fonts on manual elements
- NEVER hardcode text colors or font sizes — ALL text must use zcat text styles imported by key (`✅ Headlines/H5`, `✅ Body/Body 1-4`, `✅ Body/Subtitle 1-3`, `✅ Code Text/*`) and color variables (`BODY/Text/Static/Primary`, `BODY/Text/Static/Secondary`, `BODY/Text/Static/Disable`). Font family is **Inter** (Roboto Mono for code). NEVER write raw hex like #000000 for text. NEVER set `fontSize` or `fontName` directly instead of binding a text style
...
- Removing a required feature needs APPROVAL, not just a mention — composition freedom covers HOW required information is presented, never WHETHER required functionality exists. For any required element (a chart, a field, a column, an action) you may reposition it, resize it, change its visual treatment, merge it with related information, or move it to a secondary region. You may NOT delete it. If you believe it should be removed, STOP, state it as a design recommendation with your reasoning, and get explicit user approval BEFORE removing. "I removed X and told the user afterwards" is a rule violation. This applies even when removal seems obviously correct — especially then
- ZCAT assets are imported BY KEY and failures are LOUD — library variables and text styles are NOT local: `figma.variables.getLocalVariablesAsync()` and `figma.getLocalTextStylesAsync()` both return EMPTY, so any name-based lookup against them silently yields nothing and produces raw hex + Inter Regular text that looks fine and is wrong. Use `importVariableByKeyAsync` / `importStyleByKeyAsync` / `importComponentSetByKeyAsync` with keys from the key table, and THROW if a key does not resolve. NEVER use an empty `catch`, a name-lookup fallback, a hardcoded font, or a raw hex fill. If a required asset cannot be resolved, stop the build step and tell the user the exact missing key
- ZERO unbound text — every TEXT node you author MUST carry a zcat text style bound by key (`textStyleId` non-empty), a bound color variable, and correct family (Inter, or Roboto Mono for code). Headings and emphasised values MUST use a Semi Bold style (`Headlines/H5`, `Body/Subtitle 1-3`) — a larger Regular size is NOT hierarchy. A screen whose authored text is entirely Regular weight FAILS validation
- Component generation — use the CURRENT published generation only (the keys in the zcat.md key table / `componentKeyMap`). A superseded generation of many components still resolves by key but is absent from the published library index and has INCOMPATIBLE variant enums (Badge Color=Green/Red/Orange instead of Success/Danger/Warning; Checkbox `Check type`/`Status` instead of `Checked`/`State`). Legacy keys are permitted ONLY when working on pre-existing instances that already use them — never for new designs
- Create/Edit follows the Catalyst interaction pattern — Catalyst uses a popup/modal for Create and Edit. Preserve that model and optimise the composition inside it. Do NOT convert Create/Edit into a full page because composition freedom exists, and do not treat a wireframe drawn as a page as overriding the pattern (wireframes define fields, not interaction model). Escalate to the large/full-page popup pattern only for genuine need: >~8 fields, multi-step wizard with Stepper, side-by-side content, or embedded tables/code editors. For a merely tall form use Popup `type=With Scroll` — height scrolls, so never detach just to gain height. If an authoritative Catalyst reference shows a flow as a full page, follow that reference
...
- Rule rigidity — the sizing rules below exist to prevent overflow and broken reflow, NOT to forbid composition. HUG/FILL are the correct default and remain mandatory for content-bearing containers. But a **fixed dimension is legitimate when the design genuinely requires it**: an icon background square (40×40), a divider (1px), an avatar, a fixed-width side rail, a chart plot area, a deliberately constrained form column. In those cases set the fixed dimension on BOTH axes so it cannot collapse (the common bug is a 40×40 icon frame collapsing to 40×18 because only width was fixed), and keep the *parent* auto-layout. If a rule below would force a worse design, the design wins — say so in your summary rather than silently breaking the rule
...
- Badge variant properties are Type/Color/Size — NOT Style/Color/Size. Color values are SEMANTIC: `Primary/Grey/Purple/Danger/Disabled/Info/Success/Warning`. There is NO Green, Red, Amber, or Orange — map status meaning to Success/Danger/Warning/Info
- Key Value Pair has NO "Layout" property — its variants are State only (Default/Hover/Error/Disabled). For horizontal read-only Label:Value use General Details
- Tabs has NO "Count" property and NO "Pill" type — Type=Primary/Secondary Default/Secondary Small/Secondary Extra Small/Code Tab, and tab count is controlled by `Show Tab 3/4/5` booleans (max 5)
- Attention Box: `Type`=Message/Alert and `Color`=Info/Success/Warning/Danger/Default — "Error" and "Neutral" do not exist (use Danger / Default)
- Toggle uses `Toggled`=Off/On — not `State`=Off/On

...
- AvatarName column is ONLY for person data — user names, owners, assignees, emails. NEVER use AvatarName for IDs, amounts, dates, status, database names, regions, storage sizes, or any non-person data. AvatarName renders an avatar circle + 2 text lines — putting it on an ID column shows a person icon next to "1042" which is absurd. Match column types to DATA type, not to wireframe icons. Ask: "Is this column about a PERSON?" — YES → AvatarName, NO → Text/Badge/Date. See decision-rules/rules-table-columns.md
...
- Card height MUST be HUG (auto-layout) — NEVER set fixed pixel height on stat cards or info cards. Use `counterAxisSizingMode = "AUTO"` and let content determine height
